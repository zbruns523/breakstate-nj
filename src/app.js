...
let conditions=null,reliefLayer=null,controller=null,loadTimer=null,renderSeq=0;
document.getElementById('app').innerHTML=`<header><div><b>BREAKSTATE <i>NJ</i></b><span>Nearshore Intelligence · V57.2 Seafloor Relief</span></div><em id="status">CONNECTING</em></header><div class="shell"><aside><h3>DESTINATION</h3><select id="beach"><option value="">Choose a beach — map always stays free</option>${beaches.map(b=>`<option>${b[0]}</option>`).join('')}</select><div class="tabs"><button class="active" data-base="topo">TOPO</button><button data-base="off">SEAFLOOR ONLY</button></div><div id="live" class="card">Loading timestamped NOAA observations…</div><h3>DISPLAY</h3><label class="check"><input id="reliefToggle" type="checkbox" checked> Bathymetric shaded relief</label><h3>MAP BEHAVIOR</h3><p class="small">Pan and scroll anywhere at all times. Selecting a destination only flies the camera there; it never locks navigation. Click the seafloor for the source elevation/depth at that coordinate.</p><h3>DATA</h3><p class="small">The relief image is rendered directly by NOAA/NCEI from its DEM mosaic using multidirectional color hillshade. It is a historical/compiled elevation baseline, not a claim of today's sandbars. No synthetic depth or invented shoreline is added.</p><div id="source" class="source"></div></aside><main><div id="map"></div><div class="legend"><b>NOAA/NCEI ELEVATION-TINTED SHADED RELIEF</b><div class="gradient"></div><div class="ticks"><span>deep</span><span>offshore</span><span>nearshore</span><span>shore</span></div></div><section id="readout"><div class="loading">Move or zoom the map to render the seafloor…</div></section></main></div>`;
const map=L.map('map',{preferCanvas:true,zoomControl:true,minZoom:7,maxZoom:18,worldCopyJump:false}).setView([40.071,-74.025],14);
